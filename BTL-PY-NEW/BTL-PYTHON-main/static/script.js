document.addEventListener("DOMContentLoaded", () => {
  ...
  // Update settings modal to reflect current theme
...
function updateSettingsDisplay() {
  ...
  // Update setting option active states
  const lightOption = document.querySelector('.setting-option[onclick*="light"]');
  const darkOption = document.querySelector('.setting-option[onclick*="dark"]');
...
// Enhanced theme switching for settings modal buttons
function switchToTheme(theme) {
  ...
  if (currentTheme !== theme) {
    ...
    fetch(`/set_theme/${theme}`, {
      ...
      console.error('Error saving theme:', error);
      showNotification('Không thể lưu cài đặt giao diện', 'error');
    });
  }
}
...
function searchFolders(searchTerm) {
  ...
  folderItems.forEach(item => {
    ...
    if (folderName) {
      ...
      if (searchValue === '' || folderText.includes(searchValue)) {
        item.classList.remove('hidden');
        // Add highlight effect for search results
        if (searchValue !== '') {
          folderName.style.backgroundColor = 'var(--accent-primary)';
          folderName.style.color = 'white';
          folderName.style.borderRadius = 'var(--radius-sm)';
          folderName.style.padding = '2px 4px';
        } else {
          // Remove highlight when search is cleared
          folderName.style.backgroundColor = '';
          folderName.style.color = '';
          folderName.style.borderRadius = '';
          folderName.style.padding = '';
        }
      } else {
  ...
  const visibleFolders = document.querySelectorAll('.folder-item:not(.hidden)');
  let noResultsMsg = document.getElementById('no-folders-message');

  if (visibleFolders.length === 0 && searchValue !== '') {
    if (!noResultsMsg) {
      noResultsMsg = document.createElement('div');
      noResultsMsg.id = 'no-folders-message';
      noResultsMsg.className = 'no-results-message';
      noResultsMsg.innerHTML = `
        <p style="text-align: center; color: var(--text-muted); padding: var(--spacing-md); font-size: 0.875rem;">
          🔍 Không tìm thấy folder nào phù hợp
        </p>
      `;
      document.querySelector('.folder-list').appendChild(noResultsMsg);
    }
...
function startEditFolder(folderId, currentName, linkElement) {
  // Prevent navigation
  event.preventDefault();
  event.stopPropagation();

  const folderNameSpan = linkElement.querySelector('.folder-name');
  ...
  folderNameSpan.parentNode.insertBefore(input, folderNameSpan.nextSibling);
    ...
    if (newName && newName !== currentName) {
      ...
      .then(response => {
        if (response.ok) {
          ...
          // Revert on error
          cancelEdit();
          showNotification('Không thể đổi tên thư mục', 'error');
        }
      })
      .catch(error => {
        cancelEdit();
...
function openSettings() {
  ...
  if (modal) {
    updateSettingsDisplay(); // Ensure settings reflect current state
...
function closeShareModal() {
  ...
  if (modal) {
    modal.style.display = 'none';
    modal.classList.add('hidden');
    // Clear any generated share code
...
function generateShareCode() {
  ...
  .then(data => {
    if (data.success) {
      const codeInput = document.getElementById('share-code-input');
      if (codeInput) {
        codeInput.value = data.share_code;
      }
      showNotification('Đã tạo mã chia sẻ!');
    } else {
      showNotification('Không thể tạo mã chia sẻ', 'error');
    }
  })
  .catch(error => {
...
function copyShareCode() {
  ...
  if (codeInput && codeInput.value) {
    navigator.clipboard.writeText(codeInput.value).then(() => {
      showNotification('Đã sao chép mã chia sẻ!');
    }).catch(() => {
...
function searchFolders(query) {
  ...
  folderItems.forEach(item => {
    ...
    if (folderName) {
      const folderText = folderName.textContent.toLowerCase();
      const shouldShow = searchQuery === '' || folderText.includes(searchQuery);
...
// --- Code cho Reminder Modal ---

const reminderModal = document.getElementById('reminder-modal');
const reminderForm = document.getElementById('reminder-form');
const reminderNoteIdInput = document.getElementById('reminder-note-id');
const reminderTimeInput = document.getElementById('reminder-time');

// Hàm để mở modal nhắc nhở
function openReminderModal(noteId) {
    if (reminderModal) {
        reminderNoteIdInput.value = noteId; // Gán ID của note vào ô input ẩn
        
        // Cài đặt thời gian tối thiểu là thời gian hiện tại để người dùng không chọn quá khứ
        const now = new Date();
        now.setMinutes(now.getMinutes() - now.getTimezoneOffset()); // Điều chỉnh theo múi giờ
        reminderTimeInput.min = now.toISOString().slice(0, 16);
    ...
    reminderForm.addEventListener('submit', function(event) {
        ...
        .then(result => {
            ...
            if (result.status === 'success') {
                closeReminderModal(); // Đóng modal nếu thành công
            }
        })
        .catch(error => {
            console.error('Error:', error);
